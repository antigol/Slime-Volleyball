                           ...
                           const double roots[4], const size_t rootCount)
{
    ...
    for (size_t root = 0; root < rootCount; ++ root)
    {
        ...
        if (deriv != 0)
            maxerror = std::max(maxerror, std::abs(value / deriv));
        else
        {
            const double secDeriv = (12 * roots[root] + 6 * a) * roots[root] + 2 * b;
            if (secDeriv != 0)
                maxerror = std::max(maxerror, std::sqrt(std::abs(value / secDeriv)));
            else
            {
                const double thirdDeriv = 24 * roots[root] + 6 * a;
                if (thirdDeriv != 0)
                    maxerror = std::max(maxerror, std::pow(std::abs(value / thirdDeriv), 1.0/3.0));
                else
                    maxerror = std::max(maxerror, std::sqrt(std::sqrt(std::abs(value)/24)));
            }
        }
    }
